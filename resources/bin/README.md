...
这两个可执行文件**不是 ZeroWork 的代码**，是 pi 那两个工具的"外壳依赖"：  
pi 的 `find` 工具调 `fd`、`grep` 工具调 `rg`（pi 包内的  
`node_modules/@earendil-works/pi-coding-agent/dist/core/tools/find.js` 里 `ensureTool("fd")`）。
...
（`src/main/daemon/session-state.js` 的 `ensureAgentTools`，幂等、已就位不动）—— 与 docx 引擎同一套  
「不要求用户预装、不依赖运行时联网」的思路。

**目标目录是 `~/.pi/agent/bin`，不是 `~/.zerowork/bin`**（2026-09-17 踩过）：  
pi 的 `tools-manager.ts` 在模块加载时用 `getBinDir()`（= `getAgentDir()/bin`）把查找目录算死了，  
与我们传给 `createAgentSession` 的 `agentDir`（`~/.zerowork`）无关。  
所以 `session-state.js` 用 pi 公开导出的 `getAgentDir()` 推导，不拼自己的配置目录。  
...
打包分发时要把 `resources/`（含本目录）复制到产物同级位置，或设 `ZEROWORK_RESOURCES_DIR`，  
