# docx-engine — HTML → DOCX 转换引擎

## 概览

本目录是文档流水线的两个 Python 包：正向 `html_to_docx`（HTML → DOCX）与
反向 `docx_to_html`（DOCX → HTML + 图片）。两者平级、共用同一托管 venv 与
同一套「单行 JSON + exit 0/1」CLI 契约，都不经 `pip install` 使用。

引擎底层为开源 `html-for-docx` + `python-docx`，其上叠加一层补丁
...
resources/docx-engine/
  html_to_docx/        # 正向引擎包（顶层 23 个 .py + components/ 子包 6 个 .py）
  docx_to_html/        # 反向包（docx → HTML + 图片，见下文「反向模块」）
  requirements.txt     # 运行依赖（含 pytest）
...
## 反向模块 `docx_to_html`（docx → HTML + 图片）

把 .docx 提取成语义化 HTML + 图片目录，供「照这份文档的版式来」的场景使用
（`read_document` 只给纯文本，拿不到版式）。

实现上直接用 `python-docx` + stdlib `zipfile` 解析（零新增依赖），
**不引入预构建的 node 产物** —— 与「文档流水线统一走 Python 引擎」一致；
也**不做** HTML 转换质量的量化评估 —— 那部分由 `html-review` 技能承接。
...
- 环境搭建/预热的幂等状态机在 `src/main/daemon/runtimes.js` 的 `ensurePythonRuntime`（Task 2），本目录不含环境脚本。
