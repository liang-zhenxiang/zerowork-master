---
name: stock-tool
description: 条件选股/筛选股票工具 - 当用户需要按条件筛选股票时使用（如"找股价大于100的股票"、"PE低于20的股票"、"高股息低估值股票"等）。支持按价格、市盈率、市净率、ROE、涨跌幅、成交量、市值、资金流向等指标筛选，覆盖沪深/港股/美股。注意：本工具只做"选股筛选"，查询个股详情（行情、K线、财务、资金等）请用 stock-data
---
...
> **与 stock-data 的分工**：
> - **stock-tool**（本工具）：**筛选/选股** — "找出满足条件的股票列表"
> - **stock-data**：**查询个股详情** — "查某只股票的行情/K线/财务/资金等数据"

**数据源**：选股数据接口 | **支持市场**：A股（沪深）、港股、美股
...
```bash
# 基本用法
stock-tool filter "ClosePrice >= 100"
stock-tool filter "ClosePrice >= 100" 2026-03-12
stock-tool filter "ClosePrice >= 100" 2026-03-12 20

# AND 组合条件
stock-tool filter "intersect([PE_TTM > 0, PE_TTM < 20, ROETTM > 15])"

# OR 组合条件
stock-tool filter "union([ChangePCT > 5, Chg5D > 10])"

# 指定排序（按 ROE 降序）
stock-tool filter "intersect([PE_TTM > 0, PE_TTM < 15, ROETTM > 15])" 2026-03-12 20 ROETTM desc

# 港股选股（--market hk）
stock-tool filter "intersect([PeTTm > 0, PeTtm < 10, DivTTM > 5])" --market hk

# 美股选股（--market us）
stock-tool filter "intersect([PeTtm > 0, PeTtm < 30, TotalMV > 1000])" --market us

# 按板块筛选（--universe）
stock-tool filter "intersect([PE_TTM > 0, PE_TTM < 20])" 2026-03-12 20 --universe 11010001

# 输出原始 JSON
stock-tool filter "ClosePrice >= 100" --raw
```
...
```bash
# 使用预设函数（--preset）
stock-tool filter --preset MACDGoldenCross 2026-03-24 30
stock-tool filter --preset LowPE 2026-03-12 20
stock-tool filter --preset HighDividend 2026-03-12 20 --market hk

# 查看所有可用预设函数
stock-tool filter --list-presets
```
...
- ✅ 使用 `stock-tool` CLI 命令执行选股查询，AI 在内存中解析 JSON 并分析
...
联动分析：filter 选股 → 取前N只代码 → stock-data quote/finance 查详情
```
