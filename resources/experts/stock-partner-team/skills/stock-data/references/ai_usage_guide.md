...
> 1. `stock-data sector --search 华为` — 搜索板块代码
> 2. `stock-data sector <搜索到的代码>` — 查询成份股
>
...
> 指数成份股请使用 `index` 命令：`stock-data index sh000300`
...
> 排行查询：`stock-data sector --rank interval_chg_rank_sw1 --sort chg5Days`
