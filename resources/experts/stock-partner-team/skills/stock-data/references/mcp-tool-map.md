# stock-data → stock-mcp 工具映射

本技能通过 **stock-mcp** 连接器的 `data_*` 系列 MCP Tool 取数，不再运行本地 `scripts/`。
...
| 旧 CLI | 新 MCP |
|--------|--------|
| `stock-data search 茅台` | `data_search` query=茅台 |
| `stock-data quote sh600519` | `data_quote` code=sh600519 |
| `stock-data sector --search 华为` | `data_sector` mode=search query=华为 |
| `stock-data sector style_pt01801517` | `data_sector` mode=constituent code=style_pt01801517 |
| `stock-data lgt sh` | `data_connect` exchange=sh |
| `stock-data watchlist rank` | `data_stocklist` mode=rank |
