---
name: stock-tool
description: 通过 stock-mcp 连接器进行条件选股/策略选股/标签选股。触发词：筛选股票、选股、MACD金叉策略、央企有哪些、破净股、低PE高ROE。需已连接 stock-mcp 连接器。查个股详情用 stock-data，查概念成份股用 data_sector。
---

# Stock Data Tool（MCP 模式）

**作用**：通过 **stock-mcp** 连接器的 `tool_*` MCP Tool 完成三种选股——条件选股、策略选股、标签选股。

> **前置条件**：专家包已声明 `dependencies.connectors: ["stock-mcp"]`。

> **连接器已连接时（铁律）**：选股**必须且只能**调用连接器的 `tool_*` MCP Tool。**禁止**运行本地 `scripts/`、**禁止** WebSearch 替代选股/筛选结果、**禁止**使用其他 skill 或 MCP 取数。stock-tool 技能仅提供「何时调哪个 Tool」的方法论，实际取数通道只有 stock-mcp。

> **行情连接器未连接时**：无法调用 `tool_*` Tool。降级使用 WebSearch 及环境中其他可用 skill/MCP 取数，**筛选能力无法保证**；主理人会在 chat 单独提示「行情连接器未连接」。

> **工具分工**：
> - **stock-tool**（本技能）：筛选/选股——找出满足条件的股票列表
> - **stock-data**：查个股详情、板块/概念成份股（`data_sector`）
...
| XX概念有哪些股票 | ❌ 用 `data_sector`（stock-data） |
