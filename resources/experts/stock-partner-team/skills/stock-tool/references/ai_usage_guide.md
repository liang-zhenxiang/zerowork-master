...
```bash
# 低PE（PE < 15）
stock-tool filter "intersect([PE_TTM > 0, PE_TTM < 15])" --date 2026-03-12 --limit 20 --orderby PE_TTM:asc

# 高股息（股息率 > 5%）
stock-tool filter "DividendRatioTTM > 5" --date 2026-03-12 --limit 20 --orderby DividendRatioTTM:desc

# PEG策略（净利润增速 > 25%，PEG < 1）
stock-tool filter "intersect([PE_TTM > 0, NetProfitGrowRate > 25, PE_TTM / NetProfitGrowRate < 1])" --date 2026-03-12 --limit 20

# 低估值高ROE（PE < 15，ROE > 15%）
stock-tool filter "intersect([PE_TTM > 0, PE_TTM < 15, ROETTM > 15])" --date 2026-03-12 --limit 20 --orderby ROETTM:desc

# 破净股（PB < 1）
stock-tool filter "intersect([PB > 0, PB < 1])" --date 2026-03-12 --limit 20 --orderby PB:asc
```
...
stock-tool filter "intersect([MA_5 > MA_10, MA_10 > MA_20, MA_20 > MA_60])" --date 2026-03-12 --limit 20

# MACD金叉（DIF > DEA）
stock-tool filter "DIF > DEA" --date 2026-03-12 --limit 20

# KDJ超卖（KDJ_J < 15）
stock-tool filter "KDJ_J < 15" --date 2026-03-12 --limit 20 --orderby KDJ_J:asc

# RSI超卖（RSI_6 < 25）
stock-tool filter "RSI_6 < 25" --date 2026-03-12 --limit 20 --orderby RSI_6:asc

# 神奇九转绿9信号
stock-tool filter "NineTurn_9 = 1" --date 2026-03-12 --limit 20

# 布林带突破上轨
stock-tool filter "ClosePrice > BollingerUpper" --date 2026-03-12 --limit 20
```
...
```bash
# 主力净流入 > 1亿
stock-tool filter "MainNetFlow > 100000000" --date 2026-03-12 --limit 20 --orderby MainNetFlow:desc

# 主力持续流入（5/10/20日均为正）
stock-tool filter "intersect([MainNetFlow5D > 0, MainNetFlow10D > 0, MainNetFlow20D > 0])" --date 2026-03-12 --limit 20

# 主力5日流入 > 5亿
stock-tool filter "MainNetFlow5D > 500000000" --date 2026-03-12 --limit 20 --orderby MainNetFlow5D:desc

# 高换手率（> 5%）
stock-tool filter "TurnoverRate > 5" --date 2026-03-12 --limit 20 --orderby TurnoverRate:desc
```
...
```bash
# 高ROE（ROE > 20%）
stock-tool filter "ROETTM > 20" --date 2026-03-12 --limit 20 --orderby ROETTM:desc

# 高成长（营收增速 > 30%，净利润增速 > 40%）
stock-tool filter "intersect([RevenueGrowRate > 30, NetProfitGrowRate > 40])" --date 2026-03-12 --limit 20 --orderby NetProfitGrowRate:desc

# 低负债（资产负债率 < 40%）
stock-tool filter "DebtRatio < 40" --date 2026-03-12 --limit 20 --orderby DebtRatio:asc

# 正经营现金流
stock-tool filter "OCFPS > 0" --date 2026-03-12 --limit 20

# 高ROE低负债（ROE > 15%，负债率 < 50%）
stock-tool filter "intersect([ROETTM > 15, DebtRatio < 50])" --date 2026-03-12 --limit 20 --orderby ROETTM:desc
```
...
stock-tool filter "intersect([DividendRatioTTM > 4, PE_TTM > 0, PE_TTM < 12, PB > 0, PB < 1.5])" --date 2026-03-12 --limit 20 --orderby DividendRatioTTM:desc

# 白马成长（高ROE + 稳定增长）
stock-tool filter "intersect([ROETTM > 15, RevenueGrowRate > 15, NetProfitGrowRate > 15])" --date 2026-03-12 --limit 20 --orderby ROETTM:desc

# 困境反转（近期跌幅大但开始反弹）
stock-tool filter "intersect([Chg20D < -20, Chg5D > 0, Chg5D < 10])" --date 2026-03-12 --limit 20 --orderby Chg5D:desc

# 小盘价值（市值20-100亿，PE < 20）
stock-tool filter "intersect([TotalMV > 2000000000, TotalMV < 10000000000, PE_TTM > 0, PE_TTM < 20])" --date 2026-03-12 --limit 20 --orderby PE_TTM:asc

# 技术面+基本面组合（均线多头 + 低PE + 高ROE）
stock-tool filter "intersect([MA_5 > MA_10, MA_10 > MA_20, PE_TTM > 0, PE_TTM < 25, ROETTM > 12])" --date 2026-03-12 --limit 20 --orderby ROETTM:desc

# 次新股高成长（上市1年内 + 高增长）
stock-tool filter "intersect([ListDate > 20250317, RevenueGrowRate > 30])" --date 2026-03-12 --limit 20
```
...
```bash
# 港股高机构评级（买入评级 >= 8家）
stock-tool filter "BuyRatingNum >= 8" --date 2026-03-12 --limit 20 --orderby BuyRatingNum:desc --market hk

# 港股目标价上行空间（> 30%）
stock-tool filter "TargetPriceUpside > 30" --date 2026-03-12 --limit 20 --orderby TargetPriceUpside:desc --market hk

# 美股高机构评级
stock-tool filter "BuyRatingNum >= 8" --date 2026-03-12 --limit 20 --orderby BuyRatingNum:desc --market us

# 港股低估值 + 高评级
stock-tool filter "intersect([PeTTM > 0, PeTTM < 15, BuyRatingNum >= 5])" --date 2026-03-12 --limit 20 --market hk
```

### 3.7 按板块筛选

使用 `--universe` 限定选股范围，板块代码通过 `stock-data search <关键词> sector` 获取（去掉 `pt` 前缀）：

```bash
# 在华为概念板块中筛选低PE股票
stock-tool filter "intersect([PE_TTM > 0, PE_TTM < 25])" --date 2026-03-12 --limit 20 --orderby PE_TTM:asc --universe 02021291

# 在人工智能板块中筛选高ROE股票
stock-tool filter "ROETTM > 15" --date 2026-03-12 --limit 20 --orderby ROETTM:desc --universe 02003800
```

**常见板块代码**（可通过 stock-data 搜索获取最新代码）：
...
stock-tool filter "intersect([PeTTM > 0, PeTTM < 8, DivTTM > 6, PbLF > 0, PbLF < 0.8])" --date 2026-03-12 --limit 20 --orderby DivTTM:desc --market hk

# 港股低估值蓝筹（PE < 10，市值 > 500亿）
stock-tool filter "intersect([PeTTM > 0, PeTTM < 10, TotalMV > 500])" --date 2026-03-12 --limit 20 --orderby TotalMV:desc --market hk

# 美股低估值高股息
stock-tool filter "intersect([PeTTM > 0, PeTTM < 15, DivTTM > 3])" --date 2026-03-12 --limit 20 --orderby DivTTM:desc --market us
```
...
```bash
# 查看央企公司
stock-tool label shareholder_central_state --date 2026-04-10

# 查看国企 + 央企（多标签）
stock-tool label shareholder_central_state,shareholder_local_state --date 2026-04-10 --limit 50

# 查看千元股
stock-tool label price_up1000 --date 2026-04-10

# 查看超大盘股
stock-tool label marketcap_super_big --date 2026-04-10

# 查看近期次新股
stock-tool label listeddate_3mons --date 2026-04-10

# 查看 ST 股票（风险提示）
stock-tool label risk_st --date 2026-04-10

# 查看破净股（使用 filter 表达式）
stock-tool filter "intersect([PB > 0, PB < 1])" --date 2026-04-10 --orderby PB:asc

# 查看高ROE股票（使用 filter 表达式）
stock-tool filter "ROETTM > 15" --date 2026-04-10 --limit 50 --orderby ROETTM:desc

# 查看业绩预增股（使用 filter 表达式）
stock-tool filter "NPParentCompanyYOY > 50" --date 2026-04-10 --orderby NPParentCompanyYOY:desc
```

**分析要点**：
- 标签选股返回的是符合该分类的股票列表，适合快速了解某类股票的整体情况
- 可以多个标签组合查询，了解交叉分类的股票
- 标签选股结果可以进一步用 `stock-data` 查询个股详情进行深度分析
- 标签选股仅支持 A 股
...
stock-tool label shareholder_central_state --date 2026-04-10 --limit 100
# → 从结果中提取股票代码列表
# → stock-data quote <央企代码列表>  # 查看行情进一步筛选
```
