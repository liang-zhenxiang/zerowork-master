# stock-tool → stock-mcp 工具映射

本技能通过 **stock-mcp** 连接器的 `tool_*` 系列 MCP Tool 完成选股，不再运行本地 `scripts/`。
...
| 概念股成份股 | ❌ 不属于本技能，用 `data_sector`（stock-data） |
...
| 旧 CLI | 新 MCP |
|--------|--------|
| `stock-tool filter "PE_TTM < 20"` | `tool_filter` expression=... |
| `stock-tool filter --preset LowPE` | `tool_filter` preset=LowPE |
| `stock-tool filter --list-presets` | `tool_list_presets` |
| `stock-tool strategy macd_golden` | `tool_strategy` names=macd_golden |
| `stock-tool strategy --list` | `tool_list_strategies` |
| `stock-tool label shareholder_central_state` | `tool_label` names=shareholder_central_state |
| `stock-tool label --list` | `tool_list_labels` |
