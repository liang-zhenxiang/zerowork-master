...
"""
处理环境特定的模板文件
从指定的环境目录中读取配置并合并到基础 ZEROWORK.md 文件中
"""
...
def process_env_template(project_root, env_type):
    ...
    """
    # 构建路径
    env_template = os.path.join(project_root, f'.templates/core/{env_type}/ZEROWORK.md')
    base_template = os.path.join(project_root, 'ZEROWORK.md')
    # 验证文件是否存在
