...
async function main() {
    ...
    if (!pluginRoot || !projectDir) {
        logger.error('[PPT Stop Hook] 错误: 需要传入 ZEROWORK_PLUGIN_ROOT 和 ZEROWORK_PROJECT_DIR 参数');
        logger.error('[PPT Stop Hook] 用法: node stop.js <ZEROWORK_PLUGIN_ROOT> <ZEROWORK_PROJECT_DIR>');
        process.exit(1);
    }

    logger.log(`[PPT Stop Hook] ZEROWORK_PLUGIN_ROOT: ${pluginRoot}`);
    logger.log(`[PPT Stop Hook] ZEROWORK_PROJECT_DIR: ${projectDir}`);
...
/**
 * 执行 HTML 转 PPTX 导出
 * @param {string} pluginRoot - 插件根目录路径 (ZEROWORK_PLUGIN_ROOT)
 * @param {string} projectDir - 项目根目录路径 (ZEROWORK_PROJECT_DIR)
