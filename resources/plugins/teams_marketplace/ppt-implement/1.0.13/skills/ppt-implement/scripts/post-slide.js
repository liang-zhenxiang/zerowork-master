...
function startServer(relativePath, pluginRoot) {
    ...
    //     return;
    // }
    // 每次写入内容都时候都重启服务，上游的 hooks 有时候不准确
    ...
    logger.log(`[startServer] ZEROWORK_PLUGIN_ROOT: ${pluginRoot}`);
...
async function main() {
    ...
    logger.log(`[PPT PostToolUse Hook] ZEROWORK_PLUGIN_ROOT: ${pluginRoot}`);
    logger.log(`[PPT PostToolUse Hook] ZEROWORK_PROJECT_DIR: ${projectDir}`);

    // 验证参数
    if (!pluginRoot || !projectDir) {
        logger.error('[PPT PostToolUse Hook] 错误: 需要传入 ZEROWORK_PLUGIN_ROOT 和 ZEROWORK_PROJECT_DIR 参数');
        logger.error('[PPT PostToolUse Hook] 用法: node post-tool-use.js <ZEROWORK_PLUGIN_ROOT> <ZEROWORK_PROJECT_DIR>');
        // 输出空 JSON 对象
        process.exit(0);
    }
