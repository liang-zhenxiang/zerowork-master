...
function captureScreenshot(pluginRoot, projectDir) {
    logger.log(`[截图] ZEROWORK_PLUGIN_ROOT: ${pluginRoot}`);
    logger.log(`[截图] ZEROWORK_PROJECT_DIR: ${projectDir}`);
...
async function main() {
    ...
    if (!pluginRoot || !projectDir) {
        logger.error('[PPT 截图] 错误: 需要传入 ZEROWORK_PLUGIN_ROOT 和 ZEROWORK_PROJECT_DIR 参数');
        logger.error('[PPT 截图] 用法: node screenshot-cover.js <ZEROWORK_PLUGIN_ROOT> <ZEROWORK_PROJECT_DIR>');
        process.exit(1);
    }

    logger.log(`[PPT 截图] ZEROWORK_PLUGIN_ROOT: ${pluginRoot}`);
    logger.log(`[PPT 截图] ZEROWORK_PROJECT_DIR: ${projectDir}`);
