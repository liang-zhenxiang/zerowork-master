...
function runSetup(pluginRoot, projectDir, genieTemplatePath) {
    try {
        ...
        // 判断 ZEROWORK_PROJECT_DIR 下是否有 frontend 目录
...
async function main() {
    ...
    if (!pluginRoot || !projectDir) {
        logger.error('[PPT Setup] 错误: 需要传入 ZEROWORK_PLUGIN_ROOT 和 ZEROWORK_PROJECT_DIR 参数');
        logger.error('[PPT Setup] 用法: node setup.js <ZEROWORK_PLUGIN_ROOT> <ZEROWORK_PROJECT_DIR>');
        process.exit(1);
    }
    ...
    logger.log(`[PPT Setup] ZEROWORK_PLUGIN_ROOT: ${pluginRoot}`);
    logger.log(`[PPT Setup] ZEROWORK_PROJECT_DIR: ${projectDir}`);
