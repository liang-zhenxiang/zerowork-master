...
>   - `node` / `gitbash`：运行期由 `src/main/daemon/runtimes.js` 下载 +
...
>   - 另有构建期脚本 `scripts/fetch-node.mjs` / `fetch-gitbash.mjs`（供企业预置 /
>     离线包用）。它们属**构建工具链**，不在本仓库中。
> 运行时来源与许可的决策记录即本文档。
...
- **获取方式**：**运行期按需下载**（2026-09-18 起，见 §6）：`src/main/daemon/runtimes.js` 的
  取件规格按「`ZEROWORK_NODE_URL` 覆盖口 → 官方 dist → npmmirror 镜像」依次尝试，下载层
  把字节写进 `.part`、**sha256 通过后才改名进位**到
  `<configDir>/runtimes/.cache/node/<version>/`，再解包进暂存目录。
  另有构建期脚本 `scripts/fetch-node.mjs`（`npm run fetch:node`），同样**不在本仓库中**。
- **校验（三处 sha256，顺序不可换）**：① 官方 `SHASUMS256.txt` 交叉核对（取不到只记
  warning，取到却不一致即中止且不下载）；② 发行物整包 sha256（钉在 `src/main/daemon/runtimes.js` 的 `NODE_PLATFORM_SPECS`）；
...
- **获取方式**：**运行期按需下载 + 运行期解包**（2026-09-18 起，见 §6）。取件规格按
  「`ZEROWORK_GITBASH_URL` 覆盖口 → GitHub Release → npmmirror 镜像」依次尝试；发行物
  sha256 通过后，**调用发行物自带的 SFX 解包器**（`<artifact> -y -o<dir>`，阶段 2 本机实测
  9584 文件 / 33 s）解进暂存目录，再把 `CORRESPONDING-SOURCE.md` 拷进实例根。
  另有构建期脚本 `scripts/fetch-gitbash.mjs`（`npm run fetch:gitbash`），同样**不在本仓库中**。
- **校验**：钉在 `src/main/daemon/runtimes.js` 的发行物 sha256（实测
...
- **Windows x64 布局（实测，2026-09-17 本机解包）**：解包后 **9584 个文件 / 389.1 MB**，根层是
  ...
  - **`bash.exe -c "git --version"` 实测输出 `git version 2.55.0.windows.5`**（版本串归一化见
    `src/main/daemon/runtimes.js` 的 `parseGitVersion`）；不加 PATH 注入时这条命令读到的是
...
1. **附许可文本与版权声明**：GPLv2 全文、Bash / MSYS2 / GNU utilities 的对应 GPL 文本、
  ...
  见 `src/main/daemon/runtimes.js` 的 `GITBASH_REQUIRED_FILES`（构建期脚本
  `scripts/fetch-gitbash.mjs` 不在本仓库中，仍留同一断言备用）。
...
5. **聚合不传染**：只把它当**独立进程**调用、不与 ZeroWork 代码链接，则属 "mere aggregation"
   （GPLv2 §2 末段 / GPLv3 §5），ZeroWork 本体可保持闭源。**前提是别把它链进来。**
6. **商标**：Git 的商标政策限制 "Git" 名称与 logo 的使用 —— 不要在 UI/宣传位用它当我们的品牌名，
   只在许可清单 / 致谢里出现（精确条款**未逐条核对**）。
   **落地形态**：运行时显示名取中性描述「Bash 与 unix 工具（托管运行时）」
   （`src/main/daemon/runtimes.js` 的 `GITBASH_LABEL`，id 仍是 `gitbash`，与 spec/代码一致）。
...
- **来源（两层）**：
  1. `uv.exe`（随包）：<https://github.com/astral-sh/uv/releases/download/0.12.15/uv-x86_64-pc-windows-msvc.zip>
     —— 由构建脚本 `scripts/fetch-uv.mjs` 拉取到 `resources/bin/uv.exe`。
     （**该脚本属构建工具链，不在本仓库中**；`resources/bin/uv.exe`
     本身若随包分发，其来源与校验记录见 `resources/bin/UV-NOTICE.md`。)
...
| `<configDir>/runtimes/<id>/<version>/` | 运行期**托管实例**（内核实现在 `src/main/daemon/runtimes.js`）：`复制载荷 → 探针 → 进位 → 复验 → manifest → current` | 首次使用 / 「重置并重新安装」 | 否（用户目录） |
...
- 探针（= 探测判据，四态 `missing` / `wrong-version` / `deps-missing` / `ready` 共用既有形状）：
  - `node`：`<实例>/node.exe --version` → `v22.23.2`（解析见 `src/main/daemon/runtimes.js`）；
...
- **注入层**（`src/main/daemon/runtimes.js` 的 `planRuntimeInjection`，SubTask 2.1.3）：启用且就绪才注入，注入的是
  PATH 前置目录 + `ZEROWORK_RUNTIMES_DIR` / `ZEROWORK_NODE_HOME` / `ZEROWORK_GITBASH_HOME`；
...
- **接线点**（2026-09-17 补，此前只有纯函数、没有生产调用点）：判据由
  `src/main/daemon/runtimes.js` 的 `planRuntimeShellInjection` 组装（读设置页同一份开关 +
  各描述符的 `resolve()`），主会话（`src/main/daemon/session-files.js` 的 `runtimeShellEnv`）与子代理
  （`src/main/daemon/command-exec.js` 子代理那一侧的表工厂）各把它交给 `createSandboxedRunner` 的 `runtimeEnv`；
  执行器在 spawn 那一刻应用 —— 进沙箱的命令走 `SandboxRunRequest.env`（`src/main/sandbox/index.js`），
  降级直连 spawn 走 `runCommand` 的 env 参数（`src/main/daemon/command-exec.js`，补丁**合并**在
...
（构建期脚本 `scripts/fetch-node.mjs` / `scripts/fetch-gitbash.mjs` + `bundled-payload`；
那两个构建脚本不在本仓库中，`bundled-payload` 的逻辑现已并入 `src/main/daemon/runtimes.js`）。
**事实更新（2026-09-18，不改本段结论）**：`bundled-payload.ts` 已被删除，其探测部分收敛为
取件部分由「artifact + download」两个模块承担（`artifact.ts` / `download.ts`，
现均已并入 `src/main/daemon/runtimes.js`；当前生效形态见 §6）。
...
4. **否决（暂不采纳）：`node` 改走运行期下载，让安装包小 95 MB。**
   ...
   **状态更新（2026-09-18）**：本条已被 §6 推翻 —— 用户确实另立了决策（三个运行时全部改按需
   ...
   第 ① 条的解法是**三个运行时共用同一条取件链路**（`artifact.ts` + `download.ts`，现并入 `src/main/daemon/runtimes.js`），不再分叉。
...
7. **否决：给 node / gitbash 也开一个 `HTML_TO_DOCX_VENV` 式的环境变量覆盖口。**
   ...
   **状态更新（2026-09-18，见 §6）**：**路径覆盖口**仍然否决（无旧路径可兼容）；但离线/内网
   现在多了一个**发行物 URL 覆盖口**（`ZEROWORK_NODE_URL` / `ZEROWORK_GITBASH_URL`）与
...
- **落地形态**：
  | 环节 | 落点 |
  |---|---|
  | 取件（下载 + 校验 + 解包） | `src/main/daemon/runtimes.js`（三道门 + HTTP / `.part` / 续传 / sha256） |
  | 各运行时的来源与解包 | `src/main/daemon/runtimes.js`（node 用 jszip 解 zip、gitbash 跑发行物自带的 SFX 解包器） |
  | 探测（只读四态） | `src/main/daemon/runtimes.js`（原 `bundled-payload` 的探测部分） |
  | 安装/进位/发布 | `src/main/daemon/runtimes.js`（`installRuntime` / `resetRuntime`；顺序不变：暂存 → 取件 → 校验 → 进位 → 复验 → manifest → **最后** current） |
  | 发行物缓存与续传点 | `<configDir>/runtimes/.cache/<id>/<version>/<发行物>[.part]` |
  | 谁来触发 | `PUSH.runtimes:install-progress` / `INVOKE.runtimes:install`（设置页按钮）→ `src/main/daemon/runtimes.js` 的 `installRuntime` |
- **无自动下载怎么保证**：`ensureRuntime`（转换前 / 会话开始那条路径）**只探不装** ——
  ...
  时刻，一跑就等于静默下载）。本仓库里只有只读快照那几项覆盖（`tests/e2e/docx-runtime.mjs`）；
  另有两组针对「纯按需的门」（断言零 spawn、缓存目录都不出现）的单测不在本仓库中。
- **下载失败/离线的可执行出路**：错误文案带体积提示与「手工放置发行物」的路径
  （`<configDir>/runtimes/.cache/<id>/<version>/<发行物>`，sha256 必须等于钉死值）；
  候选源顺序 = 环境变量覆盖口（`ZEROWORK_NODE_URL` / `ZEROWORK_GITBASH_URL`）→ 官方源 → 镜像。
...
5. **否决：首次使用/启动预热时静默自动下载（「用户总会用到，先下好」）。**
   理由：这是**用户没点任何按钮时**发生的几百 MB 流量与半成品；本项目的原则是
   不静默做事，用户需求也明确写了「任何形式的静默自动下载/自动安装都不允许」。
