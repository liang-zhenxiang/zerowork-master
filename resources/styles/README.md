...
本目录 7 个 `style-*.md` 是回复风格定义：加载器按 id 给每份文件配中文名
（见下表），用户在设置页选中的那一份会全文注入主会话系统提示词。

## 风格对照（加载器 label 的唯一出处）

加载器（`src/main/daemon/resources.js` 的 `STYLE_LABELS`）按下表给每个文件配中文名，
...
- 风格只影响表达方式（HOW），不改变事实与内容（WHAT）——元规则由组装层
  （`src/main/daemon/prompt-compose.js`）在注入时附加，不在本目录文件里。
